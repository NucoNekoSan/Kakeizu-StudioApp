...
提供者名と連絡先は `src/features/legal/publisher.ts` に設定済みです。変更する場合もこのファイルの該当行を書き換えるだけで、文書本文の修正は不要です。未設定の項目を残すと、文書ページに警告が出てテストも落ちます。

連絡先は Cloudflare Email Routing で受けて転送する構成です。**公開前に、外部からこのアドレス宛にテスト送信して転送先に届くことを確認してください。**
